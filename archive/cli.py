...
class KeyboardSuppressor:
    ...
    def start(self):
        try:
            ...
            print(f"   Toggle: {self.toggle_key.upper()}, Blocked: {len(self.suppress_keys)} keys")
...
class G75Controller:
    def __init__(self, config_file="config.json", deadzone_min=2, deadzone_max=36, polling_interval_ms=5):
        self.device = None
        ...
        self.keyboard_layout = [
            ...
            "F9", "F10", "F11", "F12", "PRTSCN", "KP7", "KP8", "KP9", "u1", "u2", "u3", "u4",
        ...
        self.drunkdeer_to_standard = {
            ...
            "F": "f", "T": "t", "Q": "q", "Z": "z", "X": "x", "C": "c",
            "V": "v", "B": "b", "H": "h", "N": "n", "P": "p", "M": "m",
            "SPACE": "space", "SHF_L": "shift", "SHF_R": "shift",
            "ESC": "esc", "TAB": "tab", "RETURN": "enter"
        }
    
    def register_key_by_index(self, keycode: int, action_func: Callable, description: str = ""):
        """Register key action by index"""
        self.key_action_map[keycode] = KeyAction(action_func, False, description)
        key_name = self.keyboard_layout[keycode] if keycode < len(self.keyboard_layout) else f"?"
        print(f"  ✓ Index {keycode:3d} ({key_name:8s}) -> {description}")
    
    def register_key_by_name(self, key_name: str, action_func: Callable, description: str = ""):
        """Register key action by name"""
        try:
    ...
    def load_config(self):
        ...
        try:
            ...
            print(f"\n📋 Loading config for: {config.get('game', 'Unknown')}")
            ...
            for action, mapping in analog_maps.items():
                ...
                # Check if using direct index
                if 'drunkdeer_index' in mapping:
                    keycode = mapping['drunkdeer_index']
                    if 'LEFT_STICK_X_POSITIVE' in controller_action:
                        self.register_key_by_index(keycode, self.action_left_stick_x_positive, description)
                    elif 'LEFT_STICK_X_NEGATIVE' in controller_action:
                        self.register_key_by_index(keycode, self.action_left_stick_x_negative, description)
                    elif 'LEFT_STICK_Y_POSITIVE' in controller_action:
                        self.register_key_by_index(keycode, self.action_left_stick_y_positive, description)
                    elif 'LEFT_STICK_Y_NEGATIVE' in controller_action:
                        self.register_key_by_index(keycode, self.action_left_stick_y_negative, description)
                    elif 'RIGHT_STICK_X_POSITIVE' in controller_action:
                        self.register_key_by_index(keycode, self.action_right_stick_x_positive, description)
                    elif 'RIGHT_STICK_X_NEGATIVE' in controller_action:
                        self.register_key_by_index(keycode, self.action_right_stick_x_negative, description)
                    elif 'RIGHT_STICK_Y_POSITIVE' in controller_action:
                        self.register_key_by_index(keycode, self.action_right_stick_y_positive, description)
                    elif 'RIGHT_STICK_Y_NEGATIVE' in controller_action:
                        self.register_key_by_index(keycode, self.action_right_stick_y_negative, description)
                    elif 'RIGHT_TRIGGER' in controller_action:
                        self.register_key_by_index(keycode, self.action_right_trigger, description)
                    elif 'LEFT_TRIGGER' in controller_action:
                        self.register_key_by_index(keycode, self.action_left_trigger, description)
                    continue
                
                # Standard key mapping
                if 'drunkdeer_key' not in mapping:
                    continue
                ...
                standard_key = self.drunkdeer_to_standard.get(key_name, 'unknown')
            ...
            for action, mapping in button_maps.items():
                ...
                if 'A_BUTTON' in controller_button:
                    self.register_key_by_name(key_name, self.action_button_a, description)
                elif 'B_BUTTON' in controller_button:
                    self.register_key_by_name(key_name, self.action_button_b, description)
                elif 'X_BUTTON' in controller_button:
                    self.register_key_by_name(key_name, self.action_button_x, description)
                elif 'Y_BUTTON' in controller_button:
                    self.register_key_by_name(key_name, self.action_button_y, description)
                elif 'LEFT_BUMPER' in controller_button:
                    self.register_key_by_name(key_name, self.action_left_bumper, description)
                elif 'RIGHT_BUMPER' in controller_button:
                    self.register_key_by_name(key_name, self.action_right_bumper, description)
                elif 'START' in controller_button:
                    self.register_key_by_name(key_name, self.action_start, description)
                elif 'BACK' in controller_button:
                    self.register_key_by_name(key_name, self.action_back, description)
                elif 'LEFT_STICK_CLICK' in controller_button:
                    self.register_key_by_name(key_name, self.action_left_stick_click, description)
                elif 'RIGHT_STICK_CLICK' in controller_button:
                    self.register_key_by_name(key_name, self.action_right_stick_click, description)
                elif 'DPAD_UP' in controller_button:
                    self.register_key_by_name(key_name, self.action_dpad_up, description)
                elif 'DPAD_DOWN' in controller_button:
                    self.register_key_by_name(key_name, self.action_dpad_down, description)
                elif 'DPAD_LEFT' in controller_button:
                    self.register_key_by_name(key_name, self.action_dpad_left, description)
                elif 'DPAD_RIGHT' in controller_button:
                    self.register_key_by_name(key_name, self.action_dpad_right, description)
    ...
    def run(self):
        print("=" * 70)
        print("DrunkDeer -> Xbox Controller (The Crew Motorfest)")
        print("=" * 70)
        ...
        if self.keyboard_id == 0:
            print("❌ Failed to identify keyboard.")
            return
...
def main():
    controller = G75Controller(config_file="config.json")
    controller.run()
